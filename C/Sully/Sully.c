...
char *finj = "#include <stdlib.h>%1$c#include <stdio.h>%1$cint n = %2$i;%1$cint main(){%1$cchar *fname = %3$c%4$s%3$c;%1$cchar *fcmd = %3$c%5$s%3$c;%1$cchar *finj = %3$c%6$s%3$c;%1$cchar name[100], cmd[100];%1$csprintf(name, fname, n);%1$csprintf(cmd, fcmd, n);%1$cFILE* fl = fopen(name, %3$cw%3$c);%1$cif (fl > 0) fprintf(fl, finj, 10, n - 1, 34, fname, fcmd, finj);%1$cfflush(fl);%1$csystem(cmd);%1$c}%1$c";
